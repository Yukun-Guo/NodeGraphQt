...
def auto_layout_spring(graph):
    """
    Auto layout nodes using NetworkX spring (force-directed) layout algorithm.
    This creates a natural-looking layout based on the graph structure.
    """
    try:
        import networkx as nx
    except ImportError:
        graph.message_dialog(
            'NetworkX is required for advanced auto layout.\n'
    ...
    graph.begin_undo('Auto Layout (Spring)')
    ...
    for node in filtered_nodes:
        for output_port in node.output_ports():
            for connected_port in output_port.connected_ports():
                ...
                if target_node in filtered_nodes:
                    G.add_edge(node.id, target_node.id)

    # Calculate layout using spring layout
    # k controls the optimal distance between nodes
    # iterations controls the quality of the layout
    pos = nx.spring_layout(G, k=2.0, iterations=50, scale=500)

    # Get the center of the current node positions
    node_views = [n.view for n in filtered_nodes]
    center = graph.viewer().nodes_rect_center(node_views)
...
def auto_layout_circular(graph):
    """
    Auto layout nodes using NetworkX circular layout algorithm.
    Places nodes in a circle, useful for visualizing cyclic graphs.
    """
    try:
        import networkx as nx
    except ImportError:
    ...
    graph.begin_undo('Auto Layout (Circular)')
    ...
    # Calculate layout using circular layout
...
def auto_layout_hierarchical(graph):
    """
    Auto layout nodes using a hierarchical layout algorithm.
    Uses NetworkX's shell layout for layered visualization.
    """
    try:
        import networkx as nx
    except ImportError:
    ...
    graph.begin_undo('Auto Layout (Hierarchical)')
    ...
    # Try to use spectral layout for a hierarchical appearance
    # Falls back to shell layout if spectral fails
    try:
        pos = nx.spectral_layout(G, scale=500)
    except:
        # Shell layout groups nodes by distance from a central node
        pos = nx.shell_layout(G, scale=400)
